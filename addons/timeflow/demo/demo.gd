...
func _ready() -> void:
	...
	world_input.focus_exited.connect(_on_input_blurred.bind(world_input, Timeflow.get_clock(world_clock_configuration)))
	...
	player_input.focus_exited.connect(_on_input_blurred.bind(player_input, Timeflow.get_clock(player_clock_configuration)))
	...
	enemy_input.focus_exited.connect(_on_input_blurred.bind(enemy_input, Timeflow.get_clock(enemy_clock_configuration)))
	...
	environment_input.focus_exited.connect(_on_input_blurred.bind(environment_input, Timeflow.get_clock(environment_clock_configuration)))
...
func _on_clock_time_scale_change(timescale: float, clock: TimeflowClock, input: LineEdit) -> void:
	clock.local_time_scale = timescale
	if input and is_equal_approx(timescale, int(timescale)):
		input.text = str(int(timescale))

func _on_reset_button_pressed() -> void:
	Timeflow.get_clock(player_clock_configuration).local_time_scale = 1.0
	Timeflow.get_clock(enemy_clock_configuration).local_time_scale = 1.0
	Timeflow.get_clock(environment_clock_configuration).local_time_scale = 1.0
	if world_input:
		world_input.text = "1"
	if player_input:
		player_input.text = "1"
...
func _on_input_blurred(input: LineEdit, clock: TimeflowClock) -> void:
	if input == null:
		return
